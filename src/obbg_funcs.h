...
extern void draw_instanced_flush(float alpha);
...
extern int physics_move_walkable(vec *pos, vec *vel, float dt, type_properties *tp, interpolate_z *iz);
extern Bool physics_move_inanimate(vec *pos, vec *vel, float dt, type_properties *tp, Bool on_ground, float bouncy);
extern Bool physics_move_animate(vec *pos, vec *vel, float dt, type_properties *tp, Bool on_ground, float bouncy);
...
extern void objspace_to_worldspace_flat(float world[3], objid oid, float cam_x, float cam_y);
extern void worldspace_to_objspace_flat(float world[3], objid oid, float cam_x, float cam_y);
